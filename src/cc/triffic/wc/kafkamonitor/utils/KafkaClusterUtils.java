...
public class KafkaClusterUtils {
	...
	public static List<String> findTopicPartition(String topic) {
		ZkClient zkc = zkPool.getZkClient();
		Seq<String> seq = ZkUtils.getChildren(zkc, "/kafka/brokers/topics/"
				+ topic + "/partitions");
	...
	public static boolean findTopicIsConsumer(String topic, String group) {
		ZkClient zkc = zkPool.getZkClient();
		String ownersPath = "/kafka/consumers/" + group + "/owners/" + topic;
	...
	public static OffsetZkDomain getOffset(String topic, String group,
		...
		String offsetPath = "/kafka/consumers/" + group + "/offsets/" + topic
				+ "/" + partition;
		String ownersPath = "/kafka/consumers/" + group + "/owners/" + topic
				+ "/" + partition;
	...
	private static PartitionMetadata findLeader(List<String> a_seedBrokers,
		...
		label311: for (String seed : a_seedBrokers) {
			SimpleConsumer consumer = null;
			try {
				String ip = seed.split(":")[0];
				String port = seed.split(":")[1];
				consumer = new SimpleConsumer(ip, Integer.parseInt(port),
						10000, 65536, "leaderLookup");
	...
	public static java.util.Map<String, List<String>> getConsumers() {
		...
		try {
			Seq<String> seq = ZkUtils.getChildren(zkc, "/kafka/consumers");
			List<String> listSeq = JavaConversions.seqAsJavaList(seq);
			for (String group : listSeq) {
				Seq<String> tmp = ZkUtils.getChildren(zkc, "/kafka/consumers/"
						+ group + "/owners");
	...
	public static java.util.Map<String, List<String>> getActiveTopic() {
		...
		try {
			String group;
			Seq seq = ZkUtils.getChildren(zkc, "/kafka/consumers");
	...
	public static String getAllBrokersInfo() {
		...
		if (ZkUtils.pathExists(zkc, "/kafka/brokers/ids")) {
			Seq<String> seq = ZkUtils.getChildren(zkc, "/kafka/brokers/ids");
			List<String> listSeq = JavaConversions.seqAsJavaList(seq);
			id = 0;
			for (String ids : listSeq) {
				try {
					Tuple2<?, ?> tuple = ZkUtils.readDataMaybeNull(zkc,
							"/kafka/brokers/ids/" + ids);
	...
	public static String getAllPartitions() {
		...
		if (ZkUtils.pathExists(zkc, "/kafka/brokers/topics")) {
			Seq<String> seq = ZkUtils.getChildren(zkc, "/kafka/brokers/topics");
			List<String> listSeq = JavaConversions.seqAsJavaList(seq);
			id = 0;
